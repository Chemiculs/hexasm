...
namespace Be.Windows.Forms
{
    ...
    public class ASCIIByteCharConverter : IByteCharConverter
    {
        Encoding ascii_ = Encoding.GetEncoding(437);
        /// <summary>
        /// Returns the character to display for the byte passed across.
        /// </summary>
        /// <param name="b"></param>
        ...
        public virtual char ToChar(byte b)
        {
            string encoded = ascii_.GetString(new byte[] { b });
            return encoded.Length > 0 ? ( encoded[0] == 0x00 ? '.' : encoded[0] ) : '.' ;
        }
        ...
        public virtual byte ToByte(char c)
        {
            byte[] decoded = ascii_.GetBytes(new char[] { c });

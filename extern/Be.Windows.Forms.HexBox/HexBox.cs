...
{
	public enum programming_language_type
	{
		C,
		CPP,
		CSHARP,
	...
	public class HexBox : Control
	{
		...
		class KeyInterpreter : IKeyInterpreter
		{
			...
			protected virtual bool PreProcessWmKeyDown_Back(ref Message m)
			{
				...
				else
				{
					...
					if(_hexBox._bytePos > -1) { 
						...
						long bytestozero = (sel > 0) ? sel : 1;

						for(var  i = 0 ; i < bytestozero; ++i)
							_hexBox.ByteProvider.WriteByte(startDelete + i, (byte)0x00);
						_hexBox._bytePos -= (_hexBox._bytePos > 0 ? 1 : 0);
						//_hexBox.ByteProvider.WriteByte(_hexBox._bytePos, (byte)0x00);
						//_hexBox._bytePos -= (_hexBox._bytePos > 0 ? 1 : 0);
						_hexBox.UpdateCaret();
						_hexBox.ReleaseSelection();
						_hexBox.Invalidate();
			...
			public virtual bool PreProcessWmChar(ref Message m)
			{
				...
				// this event is NOT triggered when user types in string view, onyl when type in HEX view
				if(!_hexBox.InsertActive && ( _hexBox.SelectionStart >= _hexBox.ByteProvider.Length ) )
					return true;
				...
				if (Uri.IsHexDigit(c))
				{
					...
					string sCb = currentByte.ToString("X2", System.Threading.Thread.CurrentThread.CurrentCulture);
					if (sCb.Length == 1)
						sCb = "0" + sCb;
		...
		class StringKeyInterpreter : KeyInterpreter
		{
			...
			public override bool PreProcessWmChar(ref Message m)
			{
				...
				if(!_hexBox.InsertActive && _hexBox._bytePos >= _hexBox.ByteProvider.Length)
					return false;
		...
		private bool is_cursor_init_ = false;
		...
		void OnMouseMove(object sender, MouseEventArgs e)
		{
			if (!is_cursor_init_)
			{
				cursor_ = Cursor.Current;
				is_cursor_init_ = true;
			}

			if (this._recStringView.Contains(e.Location) || this._recHex.Contains(e.Location))
			{
		...
		public long Find(FindOptions options, bool highlight)
		{
			//var startIndex = this.SelectionStart + this.SelectionLength;
			var startIndex = options.StartPosition;
			int match = 0;

			byte[] buffer1 = options.FindBuffer;
			byte[] buffer2 = options.FindBufferUpper;

			int buffer1Length = buffer1 == null ? 0 : buffer1.Length;

			_abortFind = false;

			if(options.Type != FindType.AoB) 
			{ 
				if(options.Direction == FindDirection.Forward) { 
					for (long pos = startIndex; pos < _byteProvider.Length; pos++)
					{
						...
						if (!isMatch)
						{
							pos -= match; // the second byte in the array being compared can still be a valid begin segment of the array we look for, the this equivalates to (startofwrongmatch + 1)
							match = 0;
							_findingPos = pos;
							continue;
						}
				...
				else
				{
					...
					for (long pos = startIndex - 1; pos >= 0; pos--)
					{
						if (_abortFind)
							return -2;
			...
			else
			{
				...
				short[] buffer_aob = options.AoB;
				long buffer_aob_length = buffer_aob.Length;
				...
				if(options.Direction == FindDirection.Forward) { 
					for (long pos = startIndex; pos < _byteProvider.Length; pos++)
					{
						...
						byte compareByte = _byteProvider.ReadByte(pos);
						bool bufferMatch = compareByte == (buffer_aob[match] == -1 ? compareByte : (byte)buffer_aob[match]);
						if (!bufferMatch)
						{
							pos -= match; // the second byte in the array being compared can still be a valid begin segment of the array we look for, the this equivalates to (startofwrongmatch + 1)
							match = 0;
						...
						if (match == buffer_aob_length)
						{
							long bytePos = pos - buffer_aob_length + 1;

							if (highlight)
							{ 
								Select(bytePos, buffer_aob_length);
								ScrollByteIntoView(_bytePos + _selectionLength);
								ScrollByteIntoView(_bytePos);
							}
		...
		public void Copy()
		{
			...
			var encoder = this.ByteCharConverter;

			for(var i = 0 ; i < buffer.Length; ++i)
				sBuffer += encoder.ToChar(buffer[i]);
		...
		public void CopyAsLang(programming_language_type language)
		{
			byte[] buffer = GetCopyData();
			...
			DataObject object_d = new DataObject();

			switch (language) { 
					case programming_language_type.CPP:
					{ 
						string buffer_s = "unsigned char raw_data[" + buffer.Length.ToString() + "] = { ";
			
						for(var i = 0 ; i < buffer.Length; ++i)
						{
							buffer_s += "0x" + buffer[i].ToString("X2") + ( i == (buffer.Length - 1) ? " " : ", " );
						}

						buffer_s += "}; ";

						object_d.SetData(typeof(string), buffer_s);
						break;
					}
					case programming_language_type.CSHARP:
					{
						string buffer_s = "byte[] raw_data = new byte[" + buffer.Length.ToString() + "] { ";
					...
					case programming_language_type.JAVA:
					{
						string buffer_s = "byte[] raw_data = { ";
					
						for(var i = 0 ; i < buffer.Length; ++i)
						{
							buffer_s += "(byte)0x" + buffer[i].ToString("X2") + ( i == (buffer.Length - 1) ? " " : ", " );
						}
					...
					case programming_language_type.Pascal:
					{
						string buffer_s = "raw_data: array[0.." + buffer.Length.ToString() + "] of Byte = (";
					
						for(var i = 0 ; i < buffer.Length; ++i)
						{
							buffer_s += "$" + buffer[i].ToString("X2") + ( i == (buffer.Length - 1) ? " " : ", " );
						}
					...
					case programming_language_type.RUST:
					{
						string buffer_s = "let raw_data: [u8; " + buffer.Length.ToString() + "] = [";
					...
					case programming_language_type.VB:
					{
						string buffer_s = "Dim raw_data As Byte() = New Byte() { ";
					
						for(var i = 0 ; i < buffer.Length; ++i)
						{
							buffer_s += "&H" + buffer[i].ToString("X2") + ( i == (buffer.Length - 1) ? " " : ", " );
						}
					...
					case programming_language_type.Python:
					{
						string buffer_s = "raw_data = bytearray([";
					
						for(var i = 0 ; i < buffer.Length; ++i)
						{
							buffer_s += buffer[i].ToString() + ( i == (buffer.Length - 1) ? "" : ", " );
						}
					...
					case programming_language_type.Perl:
					{
						string buffer_s = "my @raw_data = pack('C*', ";
		...
		public void PasteString()
		{
			...
			if (da.GetDataPresent(typeof(string)))
			{
				...
				buffer = new byte[sBuffer.Length];

				for(var i = 0 ; i < buffer.Length; ++i)
					buffer[i] = this.ByteCharConverter.ToByte(sBuffer[i]);
			}
			else
			{
				return;
			...
			else
			{
				var bpos_ = _bytePos;
					...
					/*
					 * for loop
					if(bpos_ + i >= this.ByteProvider.Length) {
						SetPosition(bpos_ + (i > 0 ? (i - 1) : 0));
						break;
					}

					_byteProvider.WriteByte(bpos_ + i , buffer[i]);
					SetPosition(bpos_ + i);
					*/
					var bytes_written = _byteProvider.WriteBytes(bpos_, buffer, buffer.Length);
					SetPosition(bpos_ + bytes_written);
			}
		...
		public void PasteHex()
		{
			...
			if (da.GetDataPresent(typeof(string)))
			{
				...
				string sBuffer = ((string)da.GetData(typeof(string))).Replace(" ", "");

				if( (sBuffer.Length % 2) > 0 )
					return;
				...
				{
					int x = 0;
					for(var i = 0; i < sBuffer.Length; i += 2, ++x)
					{
						buffer[x] = Convert.ToByte(sBuffer.Substring(i, 2), 16);
					}
				}
			...
			if (this.InsertActive) { 
				_byteProvider.InsertBytes(_bytePos, buffer);
				SelectionStart = _bytePos + buffer.Length;
			}
			else
			{
				var bpos_ = _bytePos;
				...
				*/
				var bytes_written = _byteProvider.WriteBytes(bpos_, buffer, buffer.Length);
				SelectionStart = _bytePos + bytes_written;
			}
		...
		void PaintLineInfo(Graphics g, long startByte, long endByte)
		{
			...
			Brush brush = new SolidBrush(this.offset_fore_color);
		...
		private static string offset_str = "offset(h)"; 
		private static string char_str = "Decoded text";

		public Color header_title_color
		{
			get { return header_title_color_; }
			set { header_title_color_ = value; }
		} Color header_title_color_;
		...
		void PaintHeaderRow(Graphics g)
		{
			...
			Brush brush_header = new SolidBrush(header_title_color);
			var offset_s_len = g.MeasureString(offset_str, Font);
			var char_str_len = g.MeasureString(char_str, Font);

			g.DrawString(offset_str, Font, brush_header, new PointF( ( ( _recLineInfo.Width / 2 ) - ( offset_s_len.Width / 2 ) ) , (float)1.0));
			g.DrawString(char_str, Font, brush_header, new PointF( (_recStringView.Left + (_recStringView.Width / 2) ) - (char_str_len.Width / 2), (float)1.0));
		}
		...
		public IByteProvider ByteProvider
		{
			...
			set
			{
				...
				_byteProvider.UndoActionQueued += new UndoActionQueued(HandleProviderUndo);
		...
		// issue with undo / redo is object lifetime calling remove, just use undo list and don't call remove at all, use a pointer and increment / decrement it when undo / redo

		public delegate void UndoPerformed(object sender, object args);
		public delegate void RedoPerformed(object sender, object args);
		...
		public bool CanUndo()
		{
			return _byteProvider.undo_actions_.Count > 0;
		}
		public bool CanRedo()
		{
			return _byteProvider.redo_actions_.Count > 0;
		}
		public void Undo() // just handle the undo / redo lists like a stack
		{
			if(!CanUndo())
				return;

			var action = _byteProvider.undo_actions_.Last();

			_byteProvider.Undo();

			SelectionStart = action.position_;
		...
		public void Redo()
		{
			...
			var action = _byteProvider.redo_actions_.Last();
		...
		private void HandleProviderUndo(object sender, HexAction action)
		{
		...
		public Color offset_fore_color
		{
			get { return _offet_fore_color;}
			set { _offet_fore_color = value; }
		} Color _offet_fore_color;
